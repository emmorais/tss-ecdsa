...
/// 2. Receiving messages sent by other participants, and passing them to the
///    participant by calling
///    [`process_single_message()`](Participant::process_single_message()).
/// 3. Sending all messages generated by
///    [`process_single_message()`](Participant::process_single_message()) to
...
///    persists[^outs]; these identifiers are used both to tag messages and
///    incorporate context into proofs. The codebase defines a single
///    `Identifier` type; this is a global, unique identifier used to tag
///    messages. The other fields (as well as the `Identifier`) are incorporated
///    into proofs using a different mechanism to define the proof context.
///
/// 2. The paper distinguishes between identifiers for sessions (keygen) and
///    sub-sessions (auxinfo and presign)[^bug]. The codebase requires the
///    calling application to select a new, unique session [`Identifier`]s at
///    three points: (1) immediately before starting a new keygen session; (2)
///    immediately before starting a new auxinfo session; (3) immediately before
///    starting a new presigning session (for use in presigning and the
///    subsequent signature).
///
///
/// 3. 🔒 In the paper, `ssid` is updated each time the participants run the
///    key-refresh subprotocol. The codebase relies on the calling application
///    to generate a new, unique `Identifier` for each new session.
///
